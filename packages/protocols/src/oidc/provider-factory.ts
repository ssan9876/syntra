 ...
 * clients change (`oidcConfigGeneration`), so the map cannot drift from the
...
/**
 * One cached Provider, with what it was built against.
 *
 * `generation` is the tenant's `oidcConfigGeneration` as read BEFORE the
 * clients and keys were loaded, so the Provider reflects at least that
 * generation (a change that lands mid-build bumps the counter past it, and the
 * next request rebuilds). Null for an entry built without one — a caller that
 * does not know the generation, such as a test reaching into the cache.
 */
...
/**
 * A per-process Provider cache.
 *
 * THE CACHE IS PER PROCESS AND THAT IS FINE, because it is validated rather
 * than trusted. Each entry remembers the tenant's `oidcConfigGeneration` it
 * was built at; the caller passes the current value on every lookup, read
 * from the tenant row it loads per request anyway, and an entry older than
 * that is rebuilt. The counter is bumped by database triggers in the same
 * transaction as every change a Provider is built from — a client, an OIDC
 * signing key, a hostname — so a change committed through ANY replica, or by
 * the key-rotation job in the worker, is seen by every other replica on its
 * next request. There is no staleness window and no broadcast to miss.
 *
 * LISTEN/NOTIFY was the other candidate and is deliberately not used: it
 * needs a session-pinned connection, which PgBouncer in transaction pooling
 * mode does not provide, and a missed notification would be a silent stale
 * signer. The generation check is correct on its own, and costs nothing — the
 * value arrives in a query the request already makes.
 *
 * A factory rather than only module state so a test can hold two caches in
 * one process — two "replicas" — and prove a change through one is observed
 * by the other.
 */
export interface ProviderCache {
  ...
  /** A local fast path only. Other replicas learn from the generation. */
  invalidate(tenantId: string): void;
  clear(): void;
}

export function createProviderCache(): ProviderCache {
  const providers = new Map<string, CacheEntry>();

  const fresh = (entry: CacheEntry, issuer: string, generation: number | undefined) =>
    // The issuer too, not only the generation: a hostname change bumps the
    // generation as well, but the issuer is what oidc-provider stamps into
    // every token, and a cached Provider answering for a different one is
    // never right whatever the counter says.
    entry.issuer === issuer &&
    (generation === undefined ||
      // `>=`, not `===`: a request that read the tenant row just before a
      // concurrent change may arrive after the rebuild, and must not throw
      // away a NEWER Provider for its older number.
      (entry.generation !== null && entry.generation >= generation));

  return {
    async providerFor(tenantId, issuer, deps, generation) {
      const cached = providers.get(tenantId);
      if (cached && fresh(cached, issuer, generation)) return cached.provider;

      const entry: CacheEntry = {
        issuer,
        generation: generation ?? null,
        provider: buildProvider(tenantId, issuer, deps),
      };
      providers.set(tenantId, entry);
      try {
        return await entry.provider;
      } catch (error) {
        // A failed build must not be cached, or one bad client configuration
        // takes the tenant's OIDC offline until a restart. Only this entry:
        // a concurrent, newer build may already have replaced it.
        if (providers.get(tenantId) === entry) providers.delete(tenantId);
        throw error;
      }
    },
    invalidate(tenantId) {
...
/** The process's own cache, which the API serves from. */
const processCache = createProviderCache();

/**
 * Drop this process's cached Provider for a tenant.
 *
 * Not what keeps other replicas correct — `oidcConfigGeneration` is — but it
 * lets the replica that made a change skip even the comparison.
 */
export function invalidateProvider(tenantId: string): void {
  processCache.invalidate(tenantId);
}
...
/**
 * One `Provider` per tenant, built lazily and cached, and rebuilt whenever
 * `generation` (the tenant's `oidcConfigGeneration`) has moved past the one
 * it was built at. See `createProviderCache`.
 *
 ...
 * metadata once. Any change to a tenant's clients bumps the tenant's
 * `oidcConfigGeneration`, so the next request — on any replica — rebuilds.
 * That is why client administration is a write that has a cache to invalidate
 * rather than a read the provider does per request.
 */
export function providerFor(
  ...
  return processCache.providerFor(tenantId, issuer, deps, generation);
}
