...
export type WriteOperation =
  ...
  | {
      ...
      /**
       * The profile's `requirePasswordChangeAtFirstSignIn`, for a connector
       * whose target can express it (`firstSignInPasswordChange` says which).
       * Absent means "not asked", and a connector that honours it treats that
       * as false: the flag is only ever turned ON by the caller saying so.
       */
