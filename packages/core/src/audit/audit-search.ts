 ...
 *
 * The correlation id filter follows one request or job -- an HR import, the
 * provisioning run it queued, that run's connector writes -- through every
 * event it recorded. It has its own partial index
...
export interface AuditSearchFilters {
  ...
  /** Exactly this correlation id: everything one request or job recorded. */
...
export function auditSearchSql(filters: AuditSearchFilters, page: AuditSearchPage): Prisma.Sql {
  ...
  if (filters.correlationId !== undefined) {
    conditions.push(Prisma.sql`"correlationId" = ${filters.correlationId}`);
  }
