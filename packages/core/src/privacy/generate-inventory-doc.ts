...
/**
 * `pnpm privacy:inventory` writes packages/core/data-inventory.md from the
...
if (process.argv.includes('--check')) {
  ...
  if (current !== rendered) {
    console.error('packages/core/data-inventory.md is stale: run `pnpm privacy:inventory`');
    process.exit(1);
  }
  console.log('packages/core/data-inventory.md is current');
} else {
