...
import {
  ...
  isRetryable,
  passwordChangeForcedAtFirstSignIn,
  provenanceActionId,
...
export interface ApplyOptions {
  ...
  transport?: Transport;
  /**
   * The deployment's PUBLIC_URL, which the one-time pickup link is built on.
   * Threaded beside `transport` and for the same reason: an apply given a
   * transport but no public URL has nothing to put in the message, so it
   * sends nothing and says so in the audit trail rather than mailing a link
   * that goes nowhere.
   */
  publicUrl?: string;
  now?: Date;
...
function toWriteOperation(
  ...
  /** The profile's `requirePasswordChangeAtFirstSignIn`. */
  ...
  for (const action of actions) {
    ...
    try {
      outcome = await applyOneAction(tenantId, provider, action, {
        ...
        targetType: prepared.target.type,
...
interface ApplyOneOptions {
  ...
  profile: {
    initialPasswordPolicy: unknown;
    initialPasswordDelivery: string;
    requirePasswordChangeAtFirstSignIn?: boolean;
  } | null;
  /** Which connector family, for whether a forced first-sign-in change is real. */
  targetType: string;
  /** The target's display name, which the pickup message names. */
  targetName: string;
...
async function applyOneAction(
  ...
  // The profile's setting, defaulting ON for a profile row read before the
  // column existed. Carried to every connector; only one that can express it
  // (`firstSignInPasswordChange`) does anything with it.
  const requirePasswordChange = options.profile?.requirePasswordChangeAtFirstSignIn ?? true;
  const operation = toWriteOperation(action, context, initialPassword, requirePasswordChange);
  if (operation === null) {
    return finish(tenantId, action.id, {
      ok: false,
      ...
      ...(action.actionType === 'create_account'
        ? {
            ...
            delivery: (options.profile?.initialPasswordDelivery ?? 'vaultOnly') as
              ...
              | 'personalEmail'
              | 'vaultOnly',
            systemName: options.targetName,
            // What the message may promise. True only where the target will
            // actually demand a new password -- see
            // `passwordChangeForcedAtFirstSignIn`.
            forcedChange: passwordChangeForcedAtFirstSignIn(options.targetType, requirePasswordChange),
            ...(options.transport === undefined ? {} : { transport: options.transport }),
            ...(options.publicUrl === undefined ? {} : { publicUrl: options.publicUrl }),
          }
        : {}),
...
interface FinishMeta {
  ...
  transport?: Transport;
  /** The base the pickup link is built on. Without it no link can be sent. */
  publicUrl?: string;
  /** The target's display name, for the message. */
  systemName?: string;
  /** Whether the target will demand a new password at first sign-in. */
...
async function finish(
  ...
  const settled = await withTenant(tenantId, async (tx) => {
     ...
     *
     * WHAT IS SENT IS A LINK, NOT THE PASSWORD. The pickup is minted here, in
     * the same transaction as the seal it points at, so there is never a link
     * to a secret that did not commit; the password itself goes nowhere but
     * the vault and the connector. See `credential-pickup.ts` for why a
     * mailbox is the wrong place for it and why the page behind the link
     * reveals nothing on a GET.
     *
     * The message is queued AFTER the commit, below: telling somebody about an
     * account before the row that says it exists has committed is the wrong
     * order to fail in.
     */
    let deliver: CredentialLinkFacts | null = null;
    ...
    if (
      ...
      const secretName = initialSecretName(meta.targetSystemId, meta.accountId);
      await putSecret(tx, meta.provider, secretName, meta.initialPassword);

      const account = await tx.targetAccount.findUnique({
        where: { id: meta.accountId },
        include: { person: { select: { givenName: true, familyName: true } } },
      });
      const delivery = meta.delivery ?? 'vaultOnly';
      const address = await resolveDeliveryAddress(tx, account?.personId ?? null, delivery);
      ...
      if (
        ...
        meta.publicUrl !== undefined
      ) {
        const pickup = await mintCredentialPickup(tx, {
          targetAccountId: meta.accountId,
          secretName,
          recipientKind: delivery,
        ...
        deliver = {
          ...
          recipientKind: delivery,
          personName:
            `${account?.person.givenName ?? ''} ${account?.person.familyName ?? ''}`.trim() ||
            'the new starter',
          systemName: meta.systemName ?? 'the new system',
          username: account?.correlationKey ?? '',
          pickupUrl: credentialPickupUrl(meta.publicUrl, pickup.token),
      ...
      await recordEvent(tx, {
        ...
        outcome: 'success',
        sourceIp: null,
        // The secret's NAME and the pickup's id. Never the secret, never the
        // link, and never the address of a person who was not sent anything.
        payload: {
          secretName,
          delivery,
  ...
  if (settled.deliver !== null && meta.transport !== undefined) {
    queueMessage(meta.transport, renderCredentialLink(settled.deliver), {
      tenantId,
      userId: null,
      // The label, never the body. The body carries a live one-time link, and
      // a delivery-failure audit row is the last place that should end up.
