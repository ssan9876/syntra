...
describe('BrandingTab', () => {
  ...
  it('sends a cleared field as null rather than an empty string', async () => {
    ...
    // Wait for the SEEDED value, not merely for the field to exist. The form
    // is populated by an effect once the fetch resolves, so clearing as soon
    // as the input renders can clear an already-empty box and then have the
    // seed arrive afterwards — which passes or fails depending on how loaded
    // the machine is. Asserting the seeded value first removes the race.
    const nameBox = await screen.findByLabelText('Name');
    await waitFor(() => expect(nameBox).toHaveValue('Acme'));
