...
| `BreakGlassAccount` | user: `userId` | retain | Which account may be used in an emergency, and who decided it. Erasure is refused while the account is live, and the designation is accountability evidence, not data about the person. |
| `BreakGlassActivation` | user: `userId` | retain | The record of emergency administrative access and its post-event review, kept as accountability evidence like the audit log it mirrors. |
...
Linked to a data subject by user: `userId`. Erasure: **retain** -- Which account may be used in an emergency, and who decided it. Erasure is refused while the account is live, and the designation is accountability evidence, not data about the person.

| Column | Category | Erasure | Notes |
| --- | --- | --- | --- |
| `userId` | identity | retained |  |
| `credentialHash` | authentication | retained | secret |
| `designatedByUserId` | identity | retained |  |
| `designatedAt` | operational | retained |  |
| `credentialIssuedAt` | operational | retained |  |

Not personal data: `id`, `tenantId`.

#### `BreakGlassActivation`

Linked to a data subject by user: `userId`. Erasure: **retain** -- The record of emergency administrative access and its post-event review, kept as accountability evidence like the audit log it mirrors.
...
| `durationMinutes` | operational | retained |  |
| `requestedAt` | operational | retained |  |
| `requestedFromIp` | authentication | retained |  |
| `activatesAt` | operational | retained |  |
| `activatedAt` | operational | retained |  |
| `activatedBy` | operational | retained |  |
| `approvedByUserId` | identity | retained |  |
| `approverStepUpAt` | operational | retained |  |
| `expiresAt` | operational | retained |  |
| `endedAt` | operational | retained |  |
| `endedByUserId` | identity | retained |  |
| `reviewStatus` | operational | retained |  |
| `reviewedByUserId` | identity | retained |  |
| `reviewedAt` | operational | retained |  |
| `reviewerStepUpAt` | operational | retained |  |
| `reviewNotes` | operational | retained |  |
...
| `absoluteExpiresAt` | authentication | retained |  |
| `revokedAt` | authentication | retained |  |
| `breakGlassActivationId` | authentication | retained |  |
...
Not personal data: `id`, `tenantId`, `credentialKey`, `kind`, `note`, `declaredExpiresAt`, `discoveredExpiresAt`, `discoveryStatus`, `discoveryMessage`, `discoveredAt`, `effectiveExpiresAt`, `alertedExpiresAt`, `alertedThresholdDays`, `alertedAt`, `createdAt`, `updatedAt`.

#### `CredentialRotation`

| Column | Category | Erasure | Notes |
| --- | --- | --- | --- |
| `stagedByUserId` | identity |  |  |
| `verifiedByUserId` | identity |  |  |
| `cutOverByUserId` | identity |  |  |
| `completedByUserId` | identity |  |  |
| `closedByUserId` | identity |  |  |
| `evidence` | operational |  |  |

Not personal data: `id`, `tenantId`, `systemKind`, `systemId`, `credentialKey`, `status`, `reason`, `stagedSecretName`, `previousSecretName`, `newExpiresAt`, `stagedAt`, `verifiedAt`, `verificationOk`, `verificationMessage`, `verificationFingerprint`, `cutOverAt`, `completedAt`, `closedAt`, `updatedAt`.
...
Not personal data: `id`, `tenantId`, `changeClass`, `operation`, `targetType`, `baseRevision`, `reason`, `status`, `requestedAt`, `expiresAt`, `decidedAt`, `deciderStepUpAt`, `decisionNote`, `closedReason`.
