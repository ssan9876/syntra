...
| 74–78 — master-key and secret operations | Recovery, secret inventory, expiry reminders and database/key-boundary runbooks are implemented. `rewrapSecrets` now safely re-wraps data keys under a next provider, with an automated rotation test; `pnpm rekey` is the operator-run wrapper, and Vault Transit and AWS KMS providers exist behind `MASTER_KEY_PROVIDER` (backlog #47). | Perform and record a staging key-rotation drill, then choose and configure the organisation's KMS/HSM provider and run the local-to-KMS migration in staging. |
