 ...
 *
 * Every line is also appended to `<mutants>.log` as it happens, because a
 * pipeline that buffers hides whether a long run is progressing or stuck.
 */
...
function vitest(testFile, pattern) {
  const result = spawnSync('npx', ['vitest', 'run', testFile, '-t', pattern], {
    encoding: 'utf8',
    shell: true,
    timeout: 300_000,
  });
...
function run() {
  ...
  for (let i = from; i < to && i < mutants.length; i += 1) {
    ...
    say(`${m.id}\t${verdict}\t${seconds}s\tfailed=${r.failed}\t${m.note ?? ''}`);
  }
